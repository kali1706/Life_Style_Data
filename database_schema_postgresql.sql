-- Lifestyle Analytics Database Schema for PostgreSQL
...
CREATE TABLE IF NOT EXISTS exercises (
    ...
    benefit TEXT,
    burns_calories_per_30min INTEGER,
...
-- Create indexes for workout_logs table
...
-- Create indexes for exercise_logs table
CREATE INDEX IF NOT EXISTS idx_exercise_logs_workout ON exercise_logs(workout_id);
CREATE INDEX IF NOT EXISTS idx_exercise_logs_name ON exercise_logs(name_of_exercise);

-- Nutrition logs table
CREATE TABLE IF NOT EXISTS nutrition_logs (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL,
    daily_meals_frequency INTEGER,
    carbs INTEGER,
    proteins INTEGER,
    fats INTEGER,
    calories INTEGER,
...
-- Create indexes for nutrition_logs table
...
CREATE TABLE IF NOT EXISTS daily_stats (
    ...
    total_workout_duration REAL DEFAULT 0,
    total_water_intake REAL DEFAULT 0,
    avg_heart_rate INTEGER,
    consistency_score INTEGER DEFAULT 0,
    caloric_balance INTEGER,
    macro_carbs_percent REAL,
    macro_proteins_percent REAL,
    macro_fats_percent REAL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    UNIQUE(user_id, date)
);
...
INSERT INTO exercises (name, benefit, burns_calories_per_30min, target_muscle_group, equipment_needed, difficulty_level, body_part, type_of_muscle, instructions)
VALUES
